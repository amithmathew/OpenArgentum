    ...
    try:
        rows = conn.execute("""
            SELECT
                ...
                SUM(CASE WHEN t.is_transfer = 0 OR t.needs_review = 1 THEN
                    CASE WHEN t.amount_cents < 0 THEN t.amount_cents ELSE 0 END
                END) as total_spend,
                SUM(CASE WHEN t.is_transfer = 0 OR t.needs_review = 1 THEN
                    CASE WHEN t.amount_cents > 0 THEN t.amount_cents ELSE 0 END
                END) as total_income,
                COUNT(CASE WHEN NOT (t.is_transfer = 1 AND t.needs_review = 0) THEN 1 END) as transaction_count,
                SUM(CASE WHEN NOT (t.is_transfer = 1 AND t.needs_review = 0) THEN t.amount_cents ELSE 0 END) as net
            FROM transactions t
            WHERE t.is_hidden = 0
            GROUP BY month
            ORDER BY month DESC
        ...
        tier_rows = conn.execute("""
            ...
            WHERE t.is_hidden = 0 AND NOT (t.is_transfer = 1 AND t.needs_review = 0)
...
def category_breakdown(date_from: Optional[str] = None, date_to: Optional[str] = None):
    conn = get_db()
    try:
        conditions = ["t.is_hidden = 0", "NOT (t.is_transfer = 1 AND t.needs_review = 0)", "t.amount_cents < 0"]
        params = []
        if date_from:
