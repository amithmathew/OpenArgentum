  ...
  const steps = [
    ...
    <div key="llm">
      ...
      {provider === 'api_key' ? (
        ...
        <div>
          <p className="text-xs mb-2" style={{ color: 'var(--color-text-secondary)' }}>
            Using Application Default Credentials. Make sure you've run <code className="text-xs px-1 rounded" style={{ backgroundColor: 'var(--color-surface-alt)' }}>gcloud auth application-default login</code>
          </p>
          <input
            className="theme-input w-full px-3 py-2 text-sm mb-3"
            value={gcpProject}
